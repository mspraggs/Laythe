...
impl<'a> Parser<'a> {
  ...
  pub fn new(stdio: StdioWrapper, source: &'a str) -> Self {
    Self {
  ...
  fn error_at(&mut self, token: Token, message: &str) {
    ...
    match token.kind {
      TokenKind::Eof => { write!(stderr, " at end").expect("Unable to write to stderr."); },
      TokenKind::Error => (),
      _ => { write!(stderr, " at {}", token.lexeme).expect("Unable to write to stderr."); }
    }
...
mod test {
  ...
  use laythe_env::{memory::Gc};
  use laythe_native::stdio::NativeStdio;
  use std::path::PathBuf;
  ...
  fn test_compile<'a>(src: String, gc: &mut Gc) -> Managed<Fun> {
    let stdio = StdioWrapper::new(Box::new(NativeStdio::default()));
