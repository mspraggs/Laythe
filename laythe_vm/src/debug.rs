...
pub fn disassemble_chunk(stdio: &mut StdioWrapper, code_chunk: &Chunk, name: &str) -> io::Result<()> {
...
pub fn disassemble_instruction(
  stdio: &mut StdioWrapper,
  chunk: &Chunk,
  ip: usize,
  last_offset: usize,
) -> io::Result<usize> {
  let stdout = stdio.stdout();
  writeln!(stdout, "{:0>4} ", ip)?;
  ...
  match instruction {
    ...
    AlignedByteCode::ListInit(arg_count) => short_instruction(stdio.stdout(), "ListInit", arg_count, offset),
    AlignedByteCode::Map => simple_instruction(stdio.stdout(), "Map", offset),
    AlignedByteCode::MapInit(arg_count) => short_instruction(stdio.stdout(), "MapInit", arg_count, offset),
    ...
    AlignedByteCode::Import(path) => constant_instruction(stdio.stdout(), "Import", chunk, path, offset),
    ...
    AlignedByteCode::SuperInvoke((constant, arg_count)) => {
      invoke_instruction(stdio.stdout(), "SuperInvoke", chunk, constant, arg_count, offset)
    }
    ...
    AlignedByteCode::GetUpvalue(slot) => byte_instruction(stdio.stdout(), "GetUpvalue", slot, offset),
    AlignedByteCode::SetUpvalue(slot) => byte_instruction(stdio.stdout(), "SetUpvalue", slot, offset),
    ...
    AlignedByteCode::JumpIfFalse(jump) => jump_instruction(stdio.stdout(), "JumpIfFalse", 1, jump, offset),
...
fn short_instruction(stdout: &mut dyn Write, name: &str, slot: u16, offset: usize) -> io::Result<usize> {
  writeln!(stdout, "{:16} {:5}", name, slot)?;
  Ok(offset)
}

/// print a byte instruction
fn byte_instruction(stdout: &mut dyn Write, name: &str, slot: u8, offset: usize) -> io::Result<usize> {
  writeln!(stdout, "{:16} {:5}", name, slot)?;
  Ok(offset)
}
